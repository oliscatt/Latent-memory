...
PROTOCOL_DEFAULTS = {
    ...
    # append_record 那支笔构造上只落在服务端配的那一个正文目录（默认语料目录的
    # timeline 层，按窗口号+日期开文件；服务端配了 --write-dir 就是它指向的目录），
    # 而「按需读取指针」是自由填写的——没有这句提醒，用户把指针指到
    # 别处（实测有人只指了 windows/），新长出来的记忆就按需读不到，且不报错。
    # 放协议层系统填：这是跟 append_record 对齐的硬约束，不是用户偏好，不问。
    "pointers_writeback": (
        "pointers", "指针必须盖住写回层",
        "这一节的指针无论怎么增改，**必须包含记忆写回的正文落点**——默认是语料目录的 "
        "timeline 层；服务端配了 --write-dir 时，落点改成那个参数指向的目录"
        "（仍在语料目录之内）。记忆写回（latent_append）按窗口号加日期在落点开新文件。"
...
# 填死的，而 append_record 会持续往正文落点写新记录——从出货起 {end} 就开始
...
时的换窗召回正是按时间新鲜度排序的，而主题线摘要恰恰是最该在开场被带回来的那种
...
def _selftest():
    ...
    #     append_record 构造上只落服务端配的那一个正文目录（缺省是语料目录的
    #     timeline 层），而「按需读取指针」
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        #    偏偏换窗召回按新鲜度排序）。所以不写死文件名断言，而是**从 README 正文

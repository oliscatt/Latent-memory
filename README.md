...
完整说明见 [`docs/自动浮现.md`](docs/自动浮现.md)。如果要写宿主适配，请先读
[`docs/自动浮现宿主接入.md`](docs/自动浮现宿主接入.md)：**不接受把隐藏入口
`latent_passive_recall` 暴露成普通模型工具的改动。**

> 所有 Issue 我们都会认真回复；迭代之后会回到原 Issue 告知结果。 跑通了也请告诉我们——成功记录和失败记录一样有用。谢谢每一位愿意反馈的人。

## 它想守住的四件事

**换窗不等于清零。** 人格文件每轮常驻，历史正文按需检索，上次会话的线索在新窗口换窗召回；
不是拿一段总结合并，冒充长期记忆。
...
**客户端只是入口。** 八个 `latent_` MCP 工具同时支持本地 stdio 与远程 HTTP；初始化器可以
产出 `CLAUDE.md`、`AGENTS.md`、Grok agent 文件或通用人格文件。

## 已经验证到哪里

- **跨会话链路不是纸面设计**：Claude 桌面端真机验证过新会话主动换窗召回，写回在完全退出
  进程后仍可检索；Claude 手机远程路线完成换窗召回、写回落盘与跨窗命中；
...
| **ChatGPT** | OpenAI Secure MCP Tunnel 把本机回环 HTTP 接进 ChatGPT 开发者模式 | **已在 State Ledger 部署真机接通读链路**：ChatGPT Work 新窗口成功只读调用 `state_session_start`，并读取跨窗接力便签；不用开放公网入站端口，但工具请求与记忆返回仍经过 OpenAI。⚠ 该证据不等于本项目 `latent_*` 八工具已验 |
...
| [`docs/自动浮现.md`](docs/自动浮现.md) | 用户与 AI——是否启用、两种模式、部署报告、回滚和反馈 |
| [`docs/自动浮现宿主接入.md`](docs/自动浮现宿主接入.md) | AI 与宿主实现者——hook、隐藏入口、生命周期与兼容判据 |
| [`docs/快速上手.md`](docs/快速上手.md) | 想自己动手的人 |
| [`docs/注入契约.md`](docs/注入契约.md) | **自己写前端的人**——人格文件怎么拼进请求才立得住（五条判据 + 四个坑）|
| [`docs/原理与架构.md`](docs/原理与架构.md) | 想理解四层数据、八工具与检索链路的人 |
...
| [`CONTRIBUTING.md`](CONTRIBUTING.md) | 想提交兼容记录、hook 示例或其他 PR 的人 |
...
| Claude Code ／ 手机（云端线程） | **纯手机、不碰电脑跑完整条路已实测**。⚠ **成色分环节记**：自动读仓库根目录的 `.mcp.json`（验的是 HTTP 型）、容器里起进程说 MCP 协议——**都已实测**；**没验的只有“云端会不会替你启动进程型配置”一格**。`.mcp.json` 只负责挂工具；换窗召回 `latent_session_start` 要脱离模型服从度，建议交给 Claude Code `SessionStart` hook 强制请求并注入首轮上下文（官方机制已核＋一例外部方案反馈，我方未复现，见《快速上手》§3b）。这只是**换窗召回**的加固，不是逐轮自动浮现；Claude Code 是否具备后者所需的三项宿主能力仍须按《自动浮现》单独判断。⚠ 写回指令要写成 `git push origin HEAD` 并**合回主干**：写死 `main` 会推那条没动过的本地 main，`Everything up-to-date`、**退出码 0，记忆一个字没上去** |
...
| Claude 桌面 chat | **已实测**（Windows 商店／MSIX 版）：当时五个业务工具可用、换窗召回 `latent_session_start` 4/4 主动调、跨进程写回也通。人格不会自动读本地文件，**要手工贴进 profile**。人格里带着「片段是风格样本不是记忆、提到过去先查库」那句时，**模型会先调 `latent_search`**（判据是看工具真被调用）；走 `use_original_as_is` 时工具会把这句随受管协议追加到出货副本，完全不经过本工具时才要自己补。⚠ 装商店版有三个不报错的坑，见《快速上手》§3c |
